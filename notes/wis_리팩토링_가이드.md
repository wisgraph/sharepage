...
type:
  - document
title: wis 리팩토링 가이드
description: 예측 가능한 코드 (AI-Friendly Code)
---
...
- **Query (질문)**: 값을 반환하며, **관찰 가능한 상태(Externally Observable State)**를 변경하지 않는다.
  - _허용 범위_: `memoization`, `read-through cache`, `lazy loading`, 비침투적 `logging`. (호출자 입장에서 결과가 항상 같다면 내부 캐싱은 허용)
...
- _이유_: 인간과 AI의 작업 기억(Working Memory) 용량을 초과하여 문맥을 놓치게 만듦.
- **조치**: 즉시 상세 분석 및 분리 대상.
...
- **Core Domain (비즈니스 로직)**: **엄격 금지**
  ...
  - _이유_: 비즈니스 의도가 모호해짐. 구체적인 동작(`calculate`, `validate`, `persist`)을 써야 함.
- **Boundary / Infrastructure (웹, DB 어댑터)**: **허용**
  - `handleHttpRequest`, `processQueueMessage`
  - _이유_: 프레임워크나 인프라 관례상 필요한 경우가 많음.
...
## 🏗️ 프로젝트 구조 및 개발 규칙 (Project Architecture & Development Rules)

이 프로젝트는 복잡한 하위 모듈 구조를 가지고 있으며, 각 폴더는 독립적인 깃헙 레포지토리로 관리됩니다. 이 규칙은 프로젝트의 무결성을 유지하기 위해 **반드시** 준수해야 합니다.

### 1. 주요 폴더 및 역할

1. **`sharepage-template`**: 깃헙페이지 원본
   - **용도**: 플러그인 및 깃헙페이지 템플릿 개발자 전용 원본 저장소.
   - **규칙**: 깃헙페이지의 기능을 수정하거나 개선할 때는 **반드시 이 폴더**의 코드를 수정해야 합니다.
2. **`sharepage-github`**: 깃헙페이지 원본의 포크(Fork) 버전
   - **용도**: 실제 사용자의 포크 환경을 모니터링하고 테스팅하기 위한 저장소.
   - **⚠️ 절대 금지**: 이 폴더 내의 코드를 **절대 직접 수정하지 마십시오**. 이 폴더는 사용자의 환경을 시뮬레이션하기 위한 것이며, 모든 기능 변경은 `sharepage-template`에서 이루어져야 합니다.
3. **`sharepage-extension`**: 크롬 확장 프로그램
   - **용도**: 전용 크롬 확장 프로그램 코드 관리 저장소.

### 2. 소스 코드 관리 및 커밋 규칙

- **독립적 저장소**: 위 세 가지 폴더(`sharepage-template`, `sharepage-github`, `sharepage-extension`)는 루트 플러그인 레포지토리의 Git 관리 대상에서 제외됩니다.
- **개별 커밋**: 코드 변경 후 커밋을 수행할 때는 반드시 해당 폴더 내부로 이동하여 각 폴더가 연결된 고유한 원격 저장소에 커밋해야 합니다.

### 3. 테스팅 및 모니터링

- **사용자 전용 테스팅**: 테스팅은 포크된 깃헙 페이지를 통해 진행됩니다.
- **AI 에이전트 제약**: 테스팅 결과는 사용자가 직접 모니터링하며, AI 에이전트는 이를 직접 확인할 수 없음을 인지하고 작업해야 합니다.
